...
namespace Convai.Scripts.Runtime.Features
{
    ...
    public class NPC2NPCConversationManager : MonoBehaviour
    {
        ...
        {
             Debug.LogError("SmallvilleIntegration component not found on this GameObject. Please add it.");
        }
        ...
        public void RelayMessage(string message, ConvaiGroupNPCController sender, bool performSwitch = true)
        {
         ...
         conversationHistory += $"{sender.CharacterName}: {message}\n";
        ...
        private void InitializeNPCGroup(NPCGroup npcGroup)
{
    ...
    // Starting the coroutine to get the topic
    StartCoroutine(smallvilleIntegration.GetConversationTopic(npc1.CharacterID, npc2.CharacterID, topic =>
    {
        // Once the topic is generated, assign it to npcGroup.topic
        npcGroup.topic = topic;
        Debug.Log($"NPC Group topic has been set to: {npcGroup.topic}");

        // Now initialize the NPCs and start the conversation
        ...
        public void EndConversation(ConvaiGroupNPCController npc)
        {
           ...
           StartCoroutine(SmallvilleIntegration.LogInteraction(
            group.GroupNPC1.CharacterID,
            group.GroupNPC2.CharacterID,
            group.topic,
            conversationHistory,
            "neutral", // Or determine emotional tone based on the conversation
            1.0f       // Impact value, adjust as needed
            ));
        ...
        private void StartConversation(NPCGroup npcGroup)
{
    if (string.IsNullOrEmpty(npcGroup.topic))
    {
        ConvaiLogger.Warn("No topic set for the NPC group.", ConvaiLogger.LogCategory.Character);
        return;
    }
    ...
    npcGroup.CurrentSpeaker.SendTextDataNPC2NPC(message);
    npcGroup.messageToRelay = message;
    ConvaiLogger.DebugLog($"Starting conversation between {npcGroup.GroupNPC1.CharacterName} and {npcGroup.GroupNPC2.CharacterName} on topic: {npcGroup.topic}",
        ConvaiLogger.LogCategory.Character);
}
